...
class FrameComposer {
    ...
    private fun drawLeicaContent(
        ...
        val textColor = config?.let {
            invertedColor(it.getFrameColor(renderParams.frameColor.toInt(), forMusicFlow = isMusicFlowMode))
    ...
    private fun getHighContrastTextColor(stripeColor: Int): Int {
        ...
        return if (luminance > 0.8f) {
            ...
            android.graphics.Color.rgb(
                (invertedR * 1.3).toInt().coerceAtMost(255),
                (invertedG * 1.3).toInt().coerceAtMost(255),
                (invertedB * 1.3).toInt().coerceAtMost(255)
            )
        } else {
