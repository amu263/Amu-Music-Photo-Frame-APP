...
class FrameComposer {
    ...
    private fun drawPremiumLeica(
        ...
        val timeText = timeFormat.format(java.util.Date())
    ...
    private fun drawCustomLeica(
        ...
        // 3. 在底部外框区域绘制内容
        drawLeicaContent(canvas, renderParams, outputWidth, height, bottomFrameHeight, frameColor, frameWidth)
    ...
    private fun invertedColor(color: Int): Int {
        ...
        val inverted = opaqueColor xor 0x00FFFFFF
        val contrast = ColorUtils.calculateContrast(inverted, opaqueColor)
        return if (contrast < 3f) {
            ColorUtils.blendARGB(inverted, 0xFF000000.toInt(), 0.35f)
        } else {
            inverted
        }
    }
